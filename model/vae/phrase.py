...
class Phrase_VAE(object):
    ...
    def _build_model(self):
        self.encoder()
        z = self._reparameterization()
        self.decoder(z)

    def encoder(self):
        with tf.variable_scope('generator_phrase_encoder'):
            x = tf.reshape(self.phrase, [-1, BAR_SIZE * PHRASE_SIZE, PITCH_SIZE, 1])
    ...
    def decoder(self, z):
        with tf.variable_scope('generator_phrase_decoder'):
            ...
            self.logits = tf.layers.conv2d_transpose(x, 1, [3, 3], padding='same', strides=2)  # (384, 96, 1)
    ...
    def loss_function(self):
        ...
        xentropy = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.phrase, logits=tf.reshape(self.logits, [-1, BAR_SIZE * PHRASE_SIZE, PITCH_SIZE]))
