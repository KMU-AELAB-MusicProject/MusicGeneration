...
class Bar_VAE(object):
    ...
    def encoder(self):
        with tf.variable_scope('encoder'):
            x = tf.reshape(self.bar, [-1, BAR_SIZE, PITCH_SIZE, 1])
    ...
    def decoder(self, z):
        with tf.variable_scope('generator_bar_decoder'):
            ...
            self.logits = tf.layers.conv2d_transpose(x, 1, [3, 3], padding='same', strides=2)  # (96, 96, 1)  
    ...
    def loss_function(self):
        ...
        xentropy = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.bar, logits=tf.reshape(self.logits, [-1, BAR_SIZE, PITCH_SIZE]))
