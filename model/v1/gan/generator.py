...
class AddArranger(object):
    ...
    def __call__(self, z, pre_phrase, bar_generator, phrase_generator, training=True, reuse=True):
        pre_phrase_feature = phrase_generator.encoder(pre_phrase)
        with tf.variable_scope('generator', reuse=reuse):
            with tf.variable_scope('phrase_generator', reuse=reuse):
                bars = [bar_generator.decoder(tf.gather(z, i, axis=1) + pre_phrase_feature) for i in range(PHRASE_SIZE)]
                phrase1 = tf.reshape(tf.concat(bars, axis=1), [-1, BAR_SIZE * PHRASE_SIZE, PITCH_SIZE, 1])
                phrase2 = tf.reshape(phrase_generator.decoder(tf.reduce_mean(z, axis=1)),
